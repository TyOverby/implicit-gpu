...
fn no_lines() {
    ...
    let output = optimize(input, 0.001, true, false);
    let expected = vec![];
...
fn segment_with_one_line() {
    ...
    let output = optimize(input, 0.001, true, false);
    let expected = vec![
        PathSegment(vec![Point { x: 0.0, y: 0.0 }, Point { x: 1.0, y: 1.0 }].into()),
    ];
    assert_same(output, expected);
}
...
fn segment_with_two_disjoint_lines() {
    ...
    let expected = vec![
        PathSegment(vec![Point { x: 0.0, y: 0.0 }, Point { x: 1.0, y: 1.0 }].into()),
        PathSegment(vec![Point { x: 3.0, y: 3.0 }, Point { x: 4.0, y: 4.0 }].into()),
    ];
    assert_same(output, expected);
}
...
fn segment_with_two_connected_lines_going_the_wrong_way() {
    ...
    let output = optimize(input, 0.001, true, false);
    let expected = vec![
        PathSegment(vec![Point { x: 2.0, y: 2.0 }, Point { x: 1.0, y: 1.0 }].into()),
        PathSegment(vec![Point { x: 0.0, y: 0.0 }, Point { x: 1.0, y: 1.0 }].into()),
    ];
    assert_same(output, expected);
}
...
fn segment_with_two_connected_lines_going_the_wrong_way_but_only_starts_is_off() {
    ...
    let output = optimize(input, 0.001, false, false);
    let expected = vec![
        PathSegment(
            vec![
                Point { x: 2.0, y: 2.0 },
