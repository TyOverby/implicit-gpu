...
pub fn run_test(paths: &Paths) -> Result<(), Vec<Error>> {
    ...
    let mut telemetry = telemetry::DumpTelemetry::new(paths.actual_dump.clone())
    ...
    let expected_paths = WalkDir::new(&paths.expected_dump)
        .into_iter()
    ...
    for expected in expected_paths {
        ...
        // Ignore all files that start with "." (LOOKING AT YOU
        // .DS_Store)
        if expected
            .file_name()
            .and_then(|o| o.to_str())
        ...
        if !actual.exists() {
            errors.push(Error::CouldNotFind {
                file: actual.to_string_lossy().into_owned(),
            });
            continue;
        }
        ...
        match extension {
            ...
            "values" => if let Err(e) = formats::field::compare(&expected, &actual) {
            ...
            "lines" => if let Err(e) = formats::lines::compare(&expected, &actual) {
            ...
            _ => errors.push(Error::UnexpectedFile {
                file: actual.to_str().unwrap().to_owned(),
            }),
        }
    }
