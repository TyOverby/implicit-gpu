...
pub fn compare(expected: &Path, actual: &Path) -> Result<(), String> {
    ...
    let mut expected = text_to_vec(
        &latin::file::read_string_utf8(expected).unwrap(),
        expected.to_string_lossy().as_ref(),
    );
    let mut actual = text_to_vec(
...
fn test_text_to_vec() {
    ...
    assert_eq!(
        text_to_vec(s, "foo"),
        vec![Line(1.0, 2.0, 3.0, 4.0), Line(5.0, 6.0, 7.0, 8.0)]
    )
}
