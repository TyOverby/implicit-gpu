...
/// 3. Connect Obvious
/// 4. Graph Stitch
pub fn connect_lines<I>(lines: I, _simplify: bool, telemetry: &mut Telemetry, tloc: TelemetryLocation) -> Vec<PathSegment>
where
    I: Into<Vec<(Point, Point)>>,
{
    ...
    ::flame::span_of("sort lines", || {
        lines.sort_by(
            |&(Point { x: a1, y: b1, .. }, Point { x: c1, y: d1, .. }), &(Point { x: a2, y: b2, .. }, Point { x: c2, y: d2, .. })| {
                (a1, b1, c1, d1).partial_cmp(&(a2, b2, c2, d2)).unwrap_or(Ordering::Equal)
            },
        )
    });
    ...
    // connect_obvious
    let lines = ::flame::span_of("connect obvious", || connect_obvious_from_dual_qt(dual_qt, EPSILON, true));
